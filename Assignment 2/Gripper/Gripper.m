...
%     end
    ...
    L(1) = Link('d',0,'a',0,'alpha',0,'offset',0,'qlim',[deg2rad(-360),deg2rad(360)]);
    L(2) = Link('d',0,'a',0.0001,'alpha',pi/2,'offset',0,'qlim',[deg2rad(-360),deg2rad(360)]);
    
    self.model = SerialLink(L,'name','Gripper');
...
function PlotAndColourRobot(self)%robot,workspace)
    ...
    for side = 1:2 
        ...
        for linkIndex = 1:2
            ...
            if side == 1
                if linkIndex == 2
                     [ faceData, vertexData, plyData{linkIndex+1}] = plyread(['RobotiqGripperExperiment_1.ply'],'tri'); %#ok<AGROW>
                %end
